...
2. Developer and CI environments sync with
   `uv sync --frozen --no-install-project --no-build --extra dev --extra server` (add further `--extra` flags
...
- `uv lock` is a no-op against committed `uv.lock` (or the updated lock is
  committed)
- `uv sync --frozen --no-install-project --no-build --extra dev --extra server` succeeds
