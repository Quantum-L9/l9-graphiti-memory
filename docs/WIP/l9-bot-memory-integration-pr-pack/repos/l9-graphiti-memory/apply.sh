...
#   path: docs/WIP/l9-bot-memory-integration-pr-pack/repos/l9-graphiti-memory/apply.sh
#   layer: documentation
...
EXPECTED_REPO="Quantum-L9/l9-graphiti-memory"
EXPECTED_SHA="18d857688c43b0e3d4d7b2d1dc4ce0eea0d866c1"
...
cp -R "$PACK_DIR/files/." .
if [[ "${SKIP_LOCKFILE:-0}" != "1" ]]; then (cd clients/typescript && npm install --package-lock-only --ignore-scripts --no-audit --no-fund); fi
if [[ "${SKIP_LOCKFILE:-0}" != "1" && -f package.json ]]; then
  npm install --package-lock-only --ignore-scripts --no-audit --no-fund
fi
# New files must be registered in this repo's own root manifest.json (and
# ALLOWED_TOP_LEVEL, applied above via files/tools/assurance/), or the
# repository's own tests/regression/test_recursive_alignment.py CI gate
# fails on every path this overlay adds.
if [[ -f tools/assurance/generate_manifest.py ]]; then
  python3 tools/assurance/generate_manifest.py
fi
trap - ERR INT TERM
