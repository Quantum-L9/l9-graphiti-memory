...
#   path: docs/WIP/l9-bot-memory-integration-pr-pack/repos/Website-Bot/apply.sh
#   layer: documentation
...
EXPECTED_REPO="Quantum-L9/Website-Bot"
EXPECTED_SHA="db5f485b1929f6d6635e7511493098ac229004b6"
...
cp -R "$PACK_DIR/files/." .
python3 "$PACK_DIR/transform.py"
rm -f src/services/llm-stub.ts
if [[ "${SKIP_LOCKFILE:-0}" != "1" ]]; then (cd packages/bot-interop && npm install --package-lock-only --ignore-scripts --no-audit --no-fund); fi
