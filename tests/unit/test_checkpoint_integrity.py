...
def test_checkpoint_digest_is_deterministic_and_signature_detects_tampering() -> None:
    ...
    first = CheckpointIntegrity.seal("1", "checkpoint-1", {"b": 2, "a": 1}, signing_key=key)
    second = CheckpointIntegrity.seal("1", "checkpoint-1", {"a": 1, "b": 2}, signing_key=key)
    ...
    assert not CheckpointIntegrity.verify(tampered, signing_key=key, require_signature=True)
